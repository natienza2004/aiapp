...
This repository contains a simple inventory management system built with **NestJS** (backend) and a lightweight **HTML/CSS/TypeScript** frontend. It stores data in a **MySQL** database (compatible with WAMP).
...
- Frontend pages:
  ...
  - dd-item.html (create item)
  - edit-item.html (update item)
- Inventory item fields:
  - id, 
ame, quantity, category, description, createdAt
...
If your credentials differ, you can override via environment variables (see below).
...
Frontend files live in rontend/ and are served statically by the NestJS backend.
...
This starter includes NestJS testing configuration:
